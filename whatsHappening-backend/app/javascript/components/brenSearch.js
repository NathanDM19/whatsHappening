...
function createGeoJSON( inputHappenings ){
  let outputGeoJSON = {};
console.log('inside createGeoJSON : ', inputHappenings);
  if ( inputHappenings ) {
    // Initialise GeoJSON
    outputGeoJSON = {
      "id": "places",
  ...
  console.log('GeoJSON : ', outputGeoJSON);
...
export default class Search extends Component {
  ...
  console.log('mouse enter');
        ...
        console.log('layer data : ', map.getLayer());
        ...
        map.setZoom(zoom);
// map.addLayer(geoJSON);
        // console.log('trying to getlayer: ', map.getLayer("places"));

      }

      // Add the map control to enable fullscreen view
      ...
      // Add the hover panels to display info when user mouse overs a marker
      // const geoJSON = createGeoJSON( this.state.nearbyHappenings );
      // map.on('load', () => {

        // console.log('Number of layers: ', map.getSource('geojson'));
        
        // map._reallyAddLayer = map.addLayer;
        // map.addLayer = function(...args) {
        // if (map.getLayer(args[0].id)) {
        //     console.warn('Layer ' + args[0].id + ' already exists!');
        // }
